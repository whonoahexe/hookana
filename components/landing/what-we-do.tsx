          ...
          >
            ...
            <div className="flex flex-col gap-6 sm:gap-10">
              <div className="flex flex-col gap-3">
                ...
                <p className="type-heading-4 text-accent-foreground font-semibold">{tab.description}</p>
              </div>
              <ul className="w-full overflow-hidden rounded-md">
                {tab.deliverables.map((item, index) => (
                  <li key={item} className={`min-h-12 px-5 py-3 sm:px-8 sm:py-4 ${index % 2 === 0 ? "bg-linear-to-r from-background to-lime-500" : "bg-linear-to-r from-lime-500 to-background"}`}>
                    <p className="type-monospaced text-left text-xs sm:text-sm text-accent-foreground leading-snug">• {item}</p>
                  </li>
                ))}
              </ul>
              <div className="flex w-full flex-col gap-4">
                {tab.stats.map((stat) => (
                  <div key={stat.label} className="flex flex-col gap-2 border-l-8 border-pink-500 bg-background px-5 py-5 sm:border-l-[12px] sm:px-6 sm:py-6 [clip-path:polygon(0_0,calc(100%-1rem)_0,100%_12%,calc(100%-1rem)_30%,100%_48%,calc(100%-1rem)_66%,100%_84%,calc(100%-1rem)_100%,0_100%)]">
                    <p className="font-mono text-[10px] sm:text-xs leading-3 text-primary uppercase">{stat.label}</p>
